...
\tadOperacion{agPknodo}{pgo/$pg$, pknodo/$nodo$}{pgo}{$nodo \not \in$ pokenodos($pg$)}
...
\tadOperacion{moverJugador}{pgo,jugador/j,coordenada/c}{pgo}{sanciones(j) $\leq$ 4 $\land$ existeCoor(mapa(pgo))}
\tadOperacion{conectarJug}{pgo, jugador/j,coordenada/c}{pgo}{sanciones(j) $\leq$ 4 $\land$ existeCoor(mapa(pgo)) $\land$ $\neg$conectado(j)}
\tadOperacion{desconectarJug}{pgo, jugador/j}{pgo}{sanciones(j) $\leq$ 4 $\land$ conectado(j)} $\newline $
\tadOperacion{capturarJug}{pGo/pgo, jugador/j, pokemon}{pGo}{$j$ $\in$ jugadores($pgo$)} $\newline $
...
%Estoy bastante seguro que esta de mas. Creo que fue reemplazada por 'jugElegido'
...
\tadOperacion{indeDeRareza}{pgo, pokemon/q}{Nat}{cuentaTotalPokemones(pgo) > \hspace{1mm}0}
\tadOperacion{divEntera}{Nat/n, Nat/m}{Nat}{m > 0}
...
% RealizaCapturas (pgo)
$ $\newline
\tadAxioma{realizarCapturas($pgo$)}{capturarPorNodo($pgo$, nodosCapturables($pgo$))}
...
\tadAxioma{cuentaTotalPokemones(agJugador($pgo$, $j$))}{\IF sanciones(j) $\leq$ 4 THEN long(capturados($j$)) + cuentaTotalPokemones($pgo$) ELSE 0 + cuentaTotalPokemones($pgo$) FI}
